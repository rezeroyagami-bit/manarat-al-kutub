...
class DownloadsService {
  ...
  Future<List<DownloadedBook>> getDownloads() async {
    ...
    for (final item in raw) {
      try {
        final download = DownloadedBook.fromMap(
          jsonDecode(item) as Map<String, dynamic>,
        );
        if (download.filePath.isNotEmpty && await File(download.filePath).exists()) {
          result.add(download);
        } else {
          changed = true;
        }
  ...
  Future<String> copyToAppDownloads({
  ...
  Future<void> _save(List<DownloadedBook> downloads) async {
    ...
    await prefs.setStringList(
      _prefsKey,
      downloads.map((item) => jsonEncode(item.toMap())).toList(),
    );
  }
}
